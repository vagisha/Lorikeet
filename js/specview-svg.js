    ...
    function createPlot(container, datasets) {
        ...
        datasets.forEach(function(series) {
            ...
            for (var i = 0; i < series.data.length; i++) {
                x.push(series.data[i][0]);
                y.push(series.data[i][1]);
                if (series.labels && series.labels[i]) {
                    text.push(series.labels[i]);
                } else {
                    text.push("");
                }
            }
            marker.color = series.color || '#bbbbbb';
            traces.push({
                x: x,
                y: y,
                type: 'bar',
                ...
                hoverinfo: 'x+y+text',
                width: 0.8,
                yaxis: (series.yaxis === 2) ? 'y2' : 'y',
                showlegend: !!series.label
            });
        });
